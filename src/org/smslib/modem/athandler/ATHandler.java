...
public class ATHandler
    ...
    public String getAllDetectedOperators() throws TimeoutException, GatewayException, IOException, InterruptedException {
        getModemDriver().write("AT+COPS=?\r");
        return (getModemDriver().getResponse());
    }
    ...
    public String putAnyMode() throws TimeoutException, GatewayException, IOException, InterruptedException {
        getModemDriver().write("AT^SYSCFG=2,0,3FFFFFFF,2,4\r");
        return (getModemDriver().getResponse());
    }
    ...
    public int sendMessage(int size, String pdu, String phone, String text)
        ...
        if (getGateway().getProtocol() == AGateway.Protocols.TEXT) {
    ...
    public String listMessages(InboundMessage.MessageClasses messageClass)
            throws TimeoutException, GatewayException, IOException, InterruptedException {
        if (getGateway().getProtocol() == AGateway.Protocols.PDU) {
    ...
    public String getMessageByIndex(int msgIndex)
            throws TimeoutException, GatewayException, IOException, InterruptedException {
    ...
    public void readStorageLocations()
        ...
        try {
            if (response.indexOf("+CPMS:") >= 0) {
                ...
                String loc;
                label92:
                for (; tokens.hasMoreTokens(); addStorageLocation(loc)) {
    ...
    public String readPhonebookLocations()
            throws GatewayException, TimeoutException, IOException, InterruptedException {
    ...
    public String readPhonebook(String loc)
    ...
    public String sendUSSDCommand(String ussdCommand)
            throws GatewayException, TimeoutException, IOException, InterruptedException {
    ...
    public String sendUSSDCommand(String ussdCommand, boolean interactive)
        ...
        ussdResponse = getModemDriver().getResponse(AGateway.AsyncEvents.USSDRESPONSE);
        if (!interactive) {
            char esc = '\033';
            command = "" + esc + '\r';
...
//        String response;
    ...
    public AGateway.AsyncEvents processUnsolicitedEvents(String response)
            throws IOException {
